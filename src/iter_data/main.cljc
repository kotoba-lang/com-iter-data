(ns iter_data.main
  "Kotodama WASM entrypoint for the iter_data clean-room actor (L4) — Clojure port.
  ...
  facts (`iter_data.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iter_data")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iter_data.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iter_data-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iter_data-compat/ready)
